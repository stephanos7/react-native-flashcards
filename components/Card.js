  ...
  render() {
    ...
    return (
      <Animated.View style={[styles.card, rotationAnimation, initialXAnimation, dynamicTopMarginStyle, conditionallyHideOverflowStyle]}>
        <Animated.Text style={[styles.question, showingAnswer || item.attempted !== "" ? {zIndex:2} : null , (showingAnswer && item.attempted === "") ? {color:"lightgrey"} :{color: DAVYS_GREY}]}>{this.props.item.question}</Animated.Text>
